...
case "${os}" in
  windows)
    case "${target}" in
      x86)
        target_text='windows-x86'
        ;;
      x86_64)
        target_text='windows-x64'
        ;;
      aarch32)
        err "not available"
        ;;
      aarch64)
        target_text='windows-arm64'
        ;;
      *)
        err "not available"
        ;;
  ...
  linux)
    case "${target}" in
      x86)
        target_text='linux-x86'
        ;;
      x86_64)
        target_text='linux-x64'
        ;;
      aarch32)
        target_text='linux-arm'
        ;;
      aarch64)
...
readonly github_root='tree-sitter'
readonly github_repo="${github_root}"
readonly url_github_base="https://github.com/${github_root}/${github_repo}/releases"
readonly url_github_latest="${url_github_base}/latest"
...
readonly url_github_version="${url_github_base}/download/${version}"

version="${version#?}"

readonly pkg_name="${github_repo}-cli"
readonly pkg_bin="${github_repo}"
readonly pkg_filename="${pkg_name}-${target_text}"
...
unzip -u "${dir_downloads}/${pkg_archive}" -d "${dir_apps}/${pkg_name}/${pkg_filename}-v${version}"
chmod +x "${dir_apps}/${pkg_name}/${pkg_filename}-v${version}/${pkg_bin}"

rm -f "${dir_bin}/${pkg_bin}"
ln -s "${dir_apps}/${pkg_name}/${pkg_filename}-v${version}/${pkg_bin}" "${dir_bin}/${pkg_bin}"
