...
pkg_name="${github_repo}"
pkg_bin="${github_repo}"

pkg_filename_has_version=false
...
case "${pkg_compression}" in
  'zip')
    ...
    unzip -u "${dir_downloads}/${pkg_archive}" -d "${dir_apps}/${pkg_name}/${pkg_filename}"
    ;;
  'tar.xz' | 'tar.gz')
