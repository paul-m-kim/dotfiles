...
# business
url_github_base="https://github.com/elixir-lsp/elixir-ls/releases"
...
pkg_name="elixir-ls"
pkg_filename="${pkg_name}-${version}"
pkg_archive="${pkg_filename}.zip"

wget -nc -P "${dir_downloads}" "${url_github_version}/${pkg_filename}.zip"
rm -rf "${dir_apps:?}/${pkg_name:?}"
mkdir -p "${dir_apps}/${pkg_name}"
unzip -u "${dir_downloads}/${pkg_archive}" -d "${dir_apps}/${pkg_name}/"

chmod +x "${dir_apps}/elixir-ls/language_server.sh"
rm -f "${dir_bin}/elixir-ls"
ln -s "${dir_apps}/elixir-ls/language_server.sh" "${dir_bin}/elixir-ls"
